...
```bash
# Leave TURSO_* UNSET → SQLite at ./checkmark.db (this is what you want locally)
...
> if you would rather not overwrite `./checkmark.db`.
...
| `NEXT_PUBLIC_APP_URL` | **Yes**     | Canonical origin. `http://localhost:3000` in dev, `https://checkmark.kabirinnovations.com` in prod. Drives canonical links, OG URLs, robots and sitemap |
| `TURSO_DATABASE_URL`  | No (dev)    | Turso/libSQL URL. **Unset → SQLite at `./checkmark.db`.** Setting it in `.env.local` points dev at prod |
| `TURSO_AUTH_TOKEN`    | No (dev)    | Turso auth token                                                                |
| `LOCAL_DATABASE_PATH` | No          | Destination SQLite path for `npm run db:sync`. Defaults to `./checkmark.db`         |
...
Verify with `sqlite3 checkmark.db ".tables"`.
...
sqlite3 checkmark.db ".schema employees"
```
...
workspace. It is backed by the `cm_ws` cookie (`en.constants.cookieWorkspace`), written from
...
Admins add e.g. `acme.com`. CheckMark generates:
...
| Variable           | Value                        | Use                                  |
| ------------------ | ---------------------------- | ------------------------------------ |
| `--brand`          | `#1B4DFF`                    | Primary buttons, links, focus         |
| `--brand-hover`    | `#1640E0`                    | Hover / pressed                       |
...
- `public/manifest-me.json` (user), `public/manifest-ws.json` (org, linked from the
  `/ws/[slug]` layout with `theme-color: #0D1B2A`)
...
- Set `NEXT_PUBLIC_APP_URL=https://checkmark.kabirinnovations.com` in production or every canonical link, OG URL,
