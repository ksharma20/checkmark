...
async function fireMidnightWarning(onFired?: () => void): Promise<void> {
  ...
  const opts = {
    ...
    icon: "/icon-192.png",
    tag: "checkmark-midnight-warning",
    requireInteraction: true,
    actions: [
  ...
  if ("serviceWorker" in navigator) {
    try {
      const reg = await navigator.serviceWorker.ready;
      await reg.showNotification("CheckMark: Auto-checkout soon", opts);
      return;
    } catch {
  ...
  if ("Notification" in window && Notification.permission === "granted") {
    new Notification("CheckMark: Auto-checkout soon", opts);
  }
}
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (Notification.permission === "granted") {
          ...
          try {
            const reg = await navigator.serviceWorker.ready;
            await reg.showNotification("CheckMark: Checked in!", {
              body: `Auto-checkout scheduled for ${midnightStr}. Have a great day!`,
              icon: "/icon-192.png",
              tag: "checkmark-checkin-confirm",
              requireInteraction: false,
              data: { url: "/me" },
            });
          } catch {
            new Notification("CheckMark: Checked in!", {
