  ...
  return (
    <section id="faq" className="relative z-10 mx-auto max-w-[1200px] px-6 py-[80px] md:px-10 md:py-[100px]">
      ...
      <div className="reveal flex max-w-[820px] flex-col gap-3">
        {faqs.map((faq, i) => (
          <div key={faq.q} className={`overflow-hidden rounded-[14px] border bg-checkmark-bg-card transition-all ${openIndex === i ? 'border-[color-mix(in_srgb,var(--checkmark-brand)_35%,transparent)]' : 'border-checkmark-border'}`}>
            <button type="button" id={`faq-trigger-${i}`} aria-expanded={openIndex === i} aria-controls={`faq-panel-${i}`} onClick={() => setOpenIndex(openIndex === i ? null : i)} className="flex w-full items-center justify-between gap-4 border-none bg-transparent px-7 py-5 text-left font-body text-sm font-semibold text-checkmark-text transition-colors hover:text-checkmark-brand-on-dark">
              {faq.q}
              <span className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full border border-[color-mix(in_srgb,var(--checkmark-brand)_20%,transparent)] bg-[color-mix(in_srgb,var(--checkmark-brand)_10%,transparent)] text-checkmark-brand-on-dark">
