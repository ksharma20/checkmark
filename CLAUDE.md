...
32. **`develop` is `origin/main` plus the commits pushed since the last squash,
    ...
    `git rebase --onto` and force-pushes with `--force-with-lease`. The cut point is
    the newest `develop` commit whose tree matches `origin/main^{tree}`, falling back
    to the merge base when `main` has moved past the squash — and the fallback is
    refused when the carry set holds merge commits, because a `develop` that is not
    linear is not in its rebuilt shape and the merge base would replay work `main`
    already has. That is what
